...
fun export (context: Context, password: String, salt: String, arch_name: String, rout: String) {
    ...
    val data_pro = JSONObject().apply {
        put("value", Base64.getEncoder().withoutPadding().encodeToString(c.doFinal("Nothingk".toByteArray())))
        put("iv", Base64.getEncoder().withoutPadding().encodeToString(c.iv))
    }
    ...
    val uri = context.contentResolver.insert(MediaStore.Files.getContentUri("external"), content_archive)

    context.contentResolver.openOutputStream(uri!!)!!.write(archive_json.toString().toByteArray())
